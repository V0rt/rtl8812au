	...
	u8 ht, bool started)
{
	...
	if (started) {
		...
		cfg80211_ch_switch_started_notify(adapter->pnetdev, &chdef, 0, 1, false);
#else
	...
	#if (LINUX_VERSION_CODE >= KERNEL_VERSION(6, 2, 0)) || defined(CONFIG_ROCKCHIP_VOP)
...
check_bss:
	...
	if (rtw_to_roam(padapter) > 0) {
		...
		#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 12, 0)
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(6, 2, 0)) || defined(CONFIG_ROCKCHIP_VOP)
		/* cfg80211_roam_info layout changed in >=6.2 kernels (bssid moved to links[])
		 * — skip roam_info.bssid for now, bss lookup handled by kernel on newer kernels */
...
void rtw_wdev_unregister(struct wireless_dev *wdev)
{
	...
	#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 11, 0)) && \
	    (LINUX_VERSION_CODE < KERNEL_VERSION(6, 1, 0) || !defined(CONFIG_ROCKCHIP_VOP)) || \
